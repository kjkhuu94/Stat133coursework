...
```{r}
# Problem 1
files100<- str_c("plot",str_pad(1:100, width = 3, side = "left", pad = "0"),".png")
files100
```
...
```{r}
pattern_a="[aA]"
states_a<- (str_subset(states, pattern_a))
length(states)-length(states_a)
```
...
```{r}
case_two=".* .*"
str_subset(states,case_two)
```

5. Which states start with `W` and end with a consonant (non-vowel)?
```{r}
case_W_consonant<-"^[W].*[^aeiouAEIOU]$"
str_subset(states,case_W_consonant)
```

6. Which states contain at least three `i`'s (e.g. Illinois)?
```{r}
case_at_least_three_i=str_count(states,"[iI]")>=3
states[which(case_at_least_three_i)]
subset(states,case_at_least_three_i)
```

7. Which states contain exactly five vowels (e.g. California)?
```{r}
case_5_vowels=str_count(states,"[AEIOUaeiou]")==5
states[which(case_5_vowels)]
```
 
8. Which states contain three consecutive vowels (e.g. Hawaii)?
```{r}
case_3_consect_vowels="^*[(aeiouAEIOU)]{3}"
str_subset(states,case_3_consect_vowels)
```
...
reverse_chars<-function(x){
  poop=substring(x, seq(1,nchar(x),1), seq(1,nchar(x),length(x)))
  reversed=rev(poop)
  return(paste(reversed,sep="",collapse=""))
}
```
...
```{r}
codons=read.csv("codons")
dna_to_aa<- function(sequence){
  if(str_detect(sequence,"[^CGAT]")==T){
    return("Invalid DNA sequence")
    }
  dna_triplet<-codons$dna_triplet
  AminoAcids<- codons$amino_acid
  names(AminoAcids)=dna_triplet
  k=substring(sequence,seq(1,nchar(sequence),3),seq(3,nchar(sequence),3))
  poop=str_replace_all(k,AminoAcids)
  return(str_c(poop,collapse=""))
}
```
