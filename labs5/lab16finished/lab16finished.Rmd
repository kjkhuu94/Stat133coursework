...
```{r}
exrate.xml<-read_xml("http://www.ecb.europa.eu/stats/eurofxref/eurofxref-hist-90d.xml")
...
1. What is the name of the root node? "gesmes" is a namespace.  They just allow you to reuse names for different reasons.  We won't worry too much about namespaces in this class. The name of the root node is xmlns
2. What is the exchange rate for the British pound on June 23? June 24? The exchange rate for the British pound is 1.1069
3. A good data scientist always puts data into context.  What is the importance of the dates in question 2?
The importance of the dates in question 2 is important because 
4. What are the names of the children of the root element? What attributes do they have?
The names of the children of of the root element are
5. What are the names of the subchildren of the root element? What attributes do they have?
The names of the subchildren of the root element are
...
```{r}
GetExchangeRates <- function(currency_string){
  xml_find_all(exrate_root, str_c("//Cube[@currency='", currency_string, "']")) %>%
  xml_attr("rate") %>% 
  as.numeric()
}
```
...
```{r}
currency_vector<-c("GBP","USD","CAD","AUD")
currency_list<-lapply(currency_vector,GetExchangeRates)
names(currency_list)=currency_vector
```
...
```{r}
exratedf<-as.data.frame(do.call(cbind,currency_list))
exratedf<- mutate(exratedf,Time=exrate_times)
```

Wrangle the data frame to produce the plot below:

```{r}
exratedf<-gather(exratedf,key=Currency,value=currency_rate,GBP,USD,CAD,AUD)
ggplot(exratedf,aes(x=Time,y=currency_rate))+geom_line(aes(color=Currency))+xlab("Date")+ylab("Exchange Rate(against Euro)")+ggtitle("Exchange Rates Over Time")
```
