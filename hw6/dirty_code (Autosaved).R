...
mpg2$make_orig  <- mpg2$make#creates a copy of column
mpg2$make_lower <- tolower(mpg2$make)#creates a copy of make lower
  mpg2$make_lowerclean <- NA
mpg2$country <- NA#Makes country column into a list of NA
mpg2[mpg2$make_lower == "cadillac", ]$make_lowerclean <- "cadillac"#renames the column to cadillac
mpg2[mpg2$make_lower == "cadillac", ]$country <- "USA"#adds name to this dataframe.
...
write.csv(subset(mpg2, year >= 2000), "mpg2-clean.csv")
 ...
 mutate(new_column=str_to_lower(x))