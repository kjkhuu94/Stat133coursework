...
simMH<- function(n,k,switch=FALSE){
  doors<- 1:n
  if (k>n-1){
    stop("The chosen number of doors cannot be greater than the total number of doors-1")
  }
  true_door= sample(doors,1)
  chosen_door=sample(doors,1)
  if(true_door==chosen_door){
    true_and_chosen<-chosen_door
  }else{
    true_and_chosen<-c(true_door,chosen_door)}
  without_true_chosen_doors<-doors[-true_and_chosen]
  closed_doors<- new_sample(without_true_chosen_doors,k)
  remaining_doors<-setdiff(without_true_chosen_doors, closed_doors)
  final_doors<-c(remaining_doors,true_door)
  if(switch==TRUE){
    chosen_door=new_sample(final_doors,1)
  }
return(chosen_door==true_door)
}
